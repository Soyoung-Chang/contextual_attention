...
class Run(object):
    def __init__(self, args):
        # Data loader
        if args.DATASET == 'CelebA':                                # default: 'CelebA'
            self.data_loader = get_loader(args.IMAGE_PATH, args.METADATA_PATH, args.CROP_SIZE, args.IMG_SIZE, args.BATCH_SIZE, args.DATASET, args.MODE)
            # IMAGE_PATH: ./data/CelebA/images, METADATA_PATH: ./data/list_attr_celeba.txt, CROP_SIZE: 256, IMG_SIZE: 128, BATCH_SIZE: 32,

        # Model hyper-parameters
        self.image_shape = args.IMG_SHAPE                           # [128, 128, 3]

        # Hyper-parameteres
        self.stage1_lambda_l1 = args.COARSE_L1_ALPHA                # 2
        self.global_wgan_loss_alpha = args.GLOBAL_WGAN_LOSS_ALPHA   # 1.
        self.wgan_gp_lambda = args.WGAN_GP_LAMBDA                   # 10
        self.gan_loss_alpha = args.GAN_LOSS_ALPHA                   # 0.001
        self.l1_loss_alpha = args.L1_LOSS_ALPHA                     # 2
        self.ae_loss_alpha = args.AE_LOSS_ALPHA                     # 2
        self.g_lr = args.G_LR                                       # 1e-4
        self.d_lr = args.D_LR                                       # 1e-4
        self.beta1 = args.BETA1                                     # 0.5
        self.beta2 = args.BETA2                                     # 0.9

        # Training settings
        self.dataset = args.DATASET                                 # 'CelbA'
        self.num_epochs = args.NUM_EPOCHS                           # 100
        self.num_epochs_decay = args.NUM_EPOCHS_DECAY               # 10
        self.num_iters = args.NUM_ITERS                             # 200000
        self.num_iters_decay = args.NUM_ITERS_DECAY                 # 100000
        self.batch_size = args.BATCH_SIZE                           # 32
        self.use_tensorboard = args.USE_TENSORBOARD                 # False
        self.pretrained_model = args.PRETRAINED_MODEL               # None
        self.d_train_repeat = args.D_TRAIN_REPEAT                   # 2

        # Test settings
        self.test_model = args.TEST_MODEL                           # 20_10000

        # Path
        self.sample_path = args.SAMPLE_PATH                         # samples
        self.model_save_path = args.MODEL_SAVE_PATH                 # models

        # Step size
        self.print_every = args.PRINT_EVERY                         # 400
        self.sample_step = args.SAMPLE_STEP                         # 400
        self.model_save_step = args.MODEL_SAVE_STEP                 # 400

        # etc
        self.make_dir()                                             # 디렉터리 생성(models, samples 디렉터리)
        self.init_network(args)                                     # 네트워크 초기화
        self.loss = {}                                              #

        if self.pretrained_model:                                   # None
            self.load_pretrained_model()

    # 디렉터리 생성(models, samples 디렉터리)
    def make_dir(self):
        if not os.path.exists(self.model_save_path):    # models 디렉터리가 없으면 생성
            os.makedirs(self.model_save_path)
        if not os.path.exists(self.sample_path):        # samples 디렉터리가 없으면 생성
    ...
    def init_network(self, args):
        ...
        # Models
        self.G = Generator()        # Generator 객체 생성 (model.py에 Generator클래스 정의)
        self.D = Discriminator()    # Discriminator 객체 생성 (model.py에 Discriminator 클래스 정의)

        # Optimizers
        self.g_optimizer = optim.Adam(self.G.parameters(), self.g_lr, [self.beta1, self.beta2]) # Adam(?, 1e-4, [0.5, 0.9])
        self.d_optimizer = optim.Adam(self.D.parameters(), self.d_lr, [self.beta1, self.beta2]) # Adam(?, 1e-4, [0.5, 0.9])

        # Loss
        self.L1 = Discounted_L1(args)   # Loss (util.py에 Discounted_L1클래스 정의)
        self.torch_L1 = nn.L1Loss()     # L1 Loss

        # etc.
        self.util = Util(args)          # Util 객체 생성 (util.py에 Util클래스 정의)
        ...
        for epoch in range(start, self.num_epochs):
            for batch, real_image in enumerate(self.data_loader): # real_image : B x 3 x H x W
                ...
                if batch % self.print_every == 0:
                    ...
                    print('=====================================================')
                    print('reconstruction loss: ', self.loss['recon'].data)
                    print('ae loss: ', self.loss['ae_loss'].data[0][0])
                    print('g loss: ', self.loss['g_loss'].data)
                    print('d loss: ', self.loss['d_loss'].data)
...
def main(_):
    ...
    cuda.set_device(args.GPU)               # 0번 GPU로 할당 변경, default:0
    print("Running on GPU : ", args.GPU)
    run = Run(args)                         # Run 객체 생성

    if args.MODE == 'train':                # default: 'train'
        run.train()                         # train 실행
    else:
        run.test()                          # test 실행
